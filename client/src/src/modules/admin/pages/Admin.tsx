...
export default function AdminReal() {
  ...
  const loadLogs = useCallback(async () => {
    try {
      ...
      const { data } = await api.get("/logs", { skipErrorToast: true } as any);
  ...
  }, [selectedTab, loadLogs]);

  const filteredLogs = useMemo(() => {
    const q = searchByTab.logs.trim().toLowerCase();
    if (!q) return logs;
    return logs.filter((l) =>
      `${l.action || ""} ${l.user || ""} ${l.entity || ""}`
        .toLowerCase()
        .includes(q)
    );
  }, [logs, searchByTab.logs]);
        ...
        <div className="space-y-3">
          {logsLoading ? (
            ...
            <div className="bg-neutral-800 rounded-2xl p-6 text-center">
              ...
              <p className="text-neutral-500 text-xs mt-1">GET /logs</p>
            </div>
              ...
              <CardContainer key={l.id}>
                <div className="flex-1 min-w-0 text-right">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    ...
                    {l.entity && <SmallBadge>{l.entity}</SmallBadge>}
                    {l.created_at && <SmallBadge>{l.created_at}</SmallBadge>}
                  </div>
                </div>
              </CardContainer>
            ))
