...
export default function Artists() {
  ...
  const loadArtists = async () => {
    try {
      ...
      setPendingInvitations(Array.isArray(pending) ? pending : []);
    } catch (err) {
  ...
  const handleLeaveCollection = async (hostId, hostName) => {
    ...
    try {
      setLeaving(hostId);
      await api.post("/users/leave-collection", { hostId });
      showToast("השתתפותך במאגר בוטלה בהצלחה", "success");
      loadArtists(); // רענון רשימת האמנים
    } catch (err) {
  ...
  const handleAcceptInvitation = async (hostId) => {
    ...
    try {
      ...
      // עדכון מיידי - הסרת ההזמנה הממתינה מהרשימה
  ...
  return (
    <div dir="rtl" className="min-h-screen text-white pb-20">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        <div className="mb-6">
          <h1 className="text-3xl font-bold mb-2">
            {pendingInvitations.length > 0 ? (
              <span className="text-yellow-400">הזמנות ממתינות</span>
            ) : (
              <span className="text-brand-orange">אמנים במאגר שלי</span>
            )}
          </h1>
          <p className="text-neutral-400 text-sm">
            {pendingInvitations.length > 0
              ? "הזמנות שקיבלת מאמנים לצפייה במאגרים שלהם"
        ...
        {!loading && !error && pendingInvitations.length > 0 && (
          <div className="space-y-4 mb-6">
            {pendingInvitations.map((invitation) => (
              <div
                key={invitation.id}
                className="bg-yellow-900/20 border border-yellow-500/50 rounded-2xl p-6"
              >
                <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-neutral-300 mb-1">
                      <strong>{invitation.full_name || "אמן"}</strong> מזמין
                      אותך להצטרף למאגר שלו
                    </p>
                    {invitation.artist_role && (
                      <p className="text-neutral-400 text-sm mb-3">
                        תפקיד: {invitation.artist_role}
                      </p>
                    )}
                    <p className="text-neutral-400 text-sm">
        ...
        {/* רשימת אמנים - מוצגת רק אם אין הזמנות ממתינות */}
        {!loading && !error && pendingInvitations.length === 0 && (
          <>
            {artists.length === 0 ? (
              <div className="bg-neutral-900 rounded-2xl p-8 text-center border border-neutral-800">
                ...
                {artists.map((artist) => (
                  ...
                  >
                    ...
                    <div className="flex-shrink-0">
                      <button
                        onClick={() =>
                          handleLeaveCollection(artist.id, artist.full_name)
                        }
                        disabled={leaving === artist.id}
                        className="flex items-center gap-2 px-4 py-2 bg-red-500 hover:bg-red-600 disabled:bg-red-700 disabled:opacity-50 text-white font-semibold rounded-lg transition-all"
                      >
                        <LogOut size={18} />
                        {leaving === artist.id ? "מבטל..." : "בטל השתתפות"}
                      </button>
                    </div>
                  </div>
                ))}
